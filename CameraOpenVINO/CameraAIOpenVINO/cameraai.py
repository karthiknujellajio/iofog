...
ap.add_argument("-p", "--prototxt", default="MobileNetSSD_deploy.prototxt", required=False, help="path to Caffe 'deploy' prototxt file")
ap.add_argument("-m", "--model", default="MobileNetSSD_deploy.caffemodel", required=False, help="path to Caffe pre-trained model")
	...
	while True:
		...
		for i in np.arange(0, detections.shape[2]):
			...
			if confidence > args["confidence"]:
				...
				pt1 = (startX, startY)
				pt2 = (endX, endY)
				dict = {"name": CLASSES[idx], "probability": confidence * 100, "corners": [str(pt1), str(pt2)]}
